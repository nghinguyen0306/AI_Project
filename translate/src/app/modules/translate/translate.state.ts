...
import { SetSignedLanguageInput, SetTranslatedSpokenText } from './translate.actions';
...
export interface TranslateStateModel {
  ...
  signedLanguageInput: string;
  translatedSpokenText: string;
}
// Define an action for saving translations
export class SaveTranslation {
  static readonly type = '[Translate] Save Translation';
  constructor(public text: string, public videoURL: string) { }
}
...
/* 
  ...
  async downloadSignedLanguageVideo({ getState }: StateContext<TranslateStateModel>): Promise<void> {
    ...
    async downloadSignedLanguageVideo({ getState }: StateContext<TranslateStateModel>): Promise<void> {
      ...
      // Create a clean and manageable filename
      let filename = spokenLanguageText.slice(0, 50); // Limit the filename length to 50 characters
      filename = filename.replace(/[^a-zA-Z0-9-_ ]/g, '').replace(/\s+/g, '_');
  
      // Initialize JSZip instance
      const zip = new JSZip();
  
      // Add the text file to the zip with the spokenLanguageText content
      zip.file(`${filename}.txt`, spokenLanguageText);
  
      // Fetch the video from the URL and add it as a blob to the zip
      try {
        ...
        alert("Error downloading video content.");
        return;
      }
  
      // Generate the zip file and start the download process
      zip.generateAsync({ type: 'blob' })
        .then((content) => {
          const zipBlob = new Blob([content], { type: 'application/zip' });
          const downloadLink = document.createElement('a');
          downloadLink.href = URL.createObjectURL(zipBlob);
          downloadLink.download = `${filename}_translation.zip`;
  ...
  @Action(SetSignedLanguageInput)
  async setSignedLanguageInput(ctx: StateContext<TranslateStateModel>, action: SetSignedLanguageInput) {
    const { signText } = action;
    const result = await this.swService.translateSignWritingToSpoken(signText, [], 'signed', 'spoken').toPromise();
    ctx.dispatch(new SetTranslatedSpokenText(result.text)); // Dispatch translated text
  }


  @Action(SetTranslatedSpokenText)
  setTranslatedSpokenText(ctx: StateContext<TranslateStateModel>, action: SetTranslatedSpokenText) {
    const { translatedText } = action;
    console.log("Setting translated text:", translatedText); // Debugging log
    ctx.patchState({ translatedSpokenText: translatedText });
  }

  @Action(SaveTranslation)
  async saveTranslation(ctx: StateContext<any>, action: SaveTranslation) {
    try {
      await this.service.saveTranslation(action.text, action.videoURL);
